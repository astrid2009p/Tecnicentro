...
class CotIGSSVehiculoController extends Controller
{
  public function download() {
    return Excel::download(new ExportQuotes(), 'CotizacionesIGSSVehiculo.xlsx');
  }

  public function index() {
    $breadcrumbs = [
      ['link'=>"/",'name'=>"Home"],['name'=>"Cotización IGSS Vehiculo"]];

      //$cliente=c::paginate(20); compact('cliente')
      $cotizacion = CotizacionEncabezado::with('detalle')->with('clientevehiculo')->get();
      return view('/cotizaciones/igssvehiculo/index', compact('cotizacion'), ['breadcrumbs' => $breadcrumbs]);
  }

  public function create() {
    $breadcrumbs = [
      ['link'=>"/",'name'=>"Home"],['link'=>"/cotizacion-vehiculo", 'name'=>"Cotización IGSS Vehiculo"], ['name' => "Nueva Cotización"]];

      $marcas = marca::all();
      $tipos = tipo::all();
      return \view('/cotizaciones/igssvehiculo/create', compact('marcas', 'tipos'), ['breadcrumbs' => $breadcrumbs]);
  }
